...
export const getDocumentDefinitionQuery = () => `
<fetch>
...
export const getDocumentRequestsByContactIdQuery = (contactId: string) => `
<fetch>
    <entity name='${DOCUMENT_TABLES.DOCUMENT_REQUEST}'>
        <filter>
            <condition attribute='${DOC_REQUEST_FIELD_NAMES.CONTACT_ID}' operator='eq' value='${contactId}'/>
        </filter>
        ${Object.values(DOC_REQUEST_FIELD_NAMES).map(column => `  <attribute name='${column}' />`)}
        <link-entity name='${DOCUMENT_TABLES.DOC_DEFINITION}' from='${DOC_DEFINITION_FIELD_NAMES.ID}' to='${
    DOC_REQUEST_FIELD_NAMES.DEFINITION
        ...
        <link-entity name='${DOCUMENT_TABLES.DOCUMENT}' from='${DOCUMENT_FIELD_NAMES.ID}' to='${
            DOC_REQUEST_FIELD_NAMES.DOCUMENT
        }'  alias='${DOCUMENT_TABLES.DOCUMENT}' link-type='inner'>
                    <attribute name='${DOCUMENT_FIELD_NAMES.ID}' />
                </link-entity> 
    </entity>
</fetch>
`;
